...
class XSinModel(KerasFeedforwardEvaluationModel):
	def __init__(self, width, args):
		super().__init__(width, args)
		self.underlyingKerasModel = XSinKerasModel(width=width)

class XSinKerasModel(keras.Model):
	def __init__(self, width: int = 1, frequency_initializer: types.Initializer = tf.keras.initializers.RandomUniform(minval=0.01, maxval=1.99, seed=None), train_frequency: bool = False):
		super().__init__()
		self.neurons = [XSinNeuron(frequency_initializer=frequency_initializer, train_frequency=train_frequency) for i in range(0, width)]
...
class XSinNeuron(tf.keras.layers.Layer):
	@typechecked
	...
	def call(self, inputs):
		return inputs + tf.math.sin(self.frequency * inputs)
